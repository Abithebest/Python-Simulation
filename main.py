...
class Agent:
    def __init__(self, x, y):
        ...
        self.thirst = 0
        self.health = 100
        self.memory_food = set()
        self.memory_water = set()
        
    def move(self, width, height):
        if self.memory_food and self.hunger > 50:
            target = random.choice(list(self.memory_food))
            self.move_toward(target)
        elif self.memory_water and self.thirst > 50:
            target = random.choice(list(self.memory_water))
            self.move_toward(target)
        else:
    ...
    def scan_area(self, world):
        food_found = self.memory_food
        water_found = self.memory_water
        ...
        self.memory_food = food_found
        self.memory_water = water_found
...
class World:
    ...
    def display_map(self, world_map):
        ...
        for agent in self.agents:
            ...
            print(f"{agent_color}{agent.name}{Colors.ENDC} | Hunger: {agent.hunger} | Thirst: {agent.thirst} | Health: {agent.health} | Memory: F({agent.memory_food}) - W({agent.memory_water})")
...
def end_time():
    ...
    print(f"{Colors.GREEN}DURATION: {duration:.4f}s")